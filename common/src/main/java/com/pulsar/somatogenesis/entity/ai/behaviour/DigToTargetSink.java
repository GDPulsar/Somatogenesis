package com.pulsar.somatogenesis.entity.ai.behaviour;
...
public class DigToTargetSink extends Behavior<Mob> {
    ...
    public DigToTargetSink() {
        this(150, 250);
    }

    public DigToTargetSink(int i, int j) {
    ...
    protected boolean canStillUse(ServerLevel serverLevel, Mob mob, long l) {
        if (this.path != null && this.lastTargetPos != null) {
            ...
            boolean bl = optional.map(DigToTargetSink::isWalkTargetSpectator).orElse(false);
    ...
    protected void stop(ServerLevel serverLevel, Mob mob, long l) {
        if (mob.getBrain().hasMemoryValue(MemoryModuleType.WALK_TARGET) && !this.reachedTarget(mob, (WalkTarget)mob.getBrain().getMemory(MemoryModuleType.WALK_TARGET).get()) && mob.getNavigation().isStuck()) {
